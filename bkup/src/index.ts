...
class TikiSdk {
  ...
  private async exportCryptoKey(key) {
    ...
    const message = await window.___TikiSdk__KeyStorageWrite(
      "mykeykey",
      exportedKeyBuffer
    );
  ...
  public async generateAddress(): Promise<string> {
    ...
    const address = await globalThis.crypto.subtle.digest(
      "SHA-256",
      new Uint8Array(array, 0, 5)
    );
  ...
  public async withId(addy: string): Promise<string> {
    ...
    const message = await window.___TikiSdk__KeyStorageRead(addy);
    return message;
  }
